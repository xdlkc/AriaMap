package com.xidian.aria.ariamap.services;
...
import com.xidian.aria.ariamap.calwalk.StepCount;
import com.xidian.aria.ariamap.listeners.StepValuePassListener;
...
public class StepService extends Service implements SensorEventListener {
    ...
    private void initTodayData() {
        ...
        //获取当天的数据，用于展示
        List<StepDo> list = DbUtils.getQueryByWhere(StepDo.class, "today", new String[]{CURRENT_DATE});
        if (list.size() == 0 || list.isEmpty()) {
            CURRENT_STEP = 0;
        } else if (list.size() == 1) {
            Log.v(TAG, "StepDo=" + list.get(0).toString());
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
     ...
     * UI监听器对象
     */
    private UpdateStepListener mCallback;
     ...
     */
    public void registerCallback(UpdateStepListener paramICallback) {
    ...
    public void onSensorChanged(SensorEvent event) {
        Log.d(TAG, "onSensorChanged: xxxxx");
    ...
    private void save() {
        ...
        List<StepDo> list = DbUtils.getQueryByWhere(StepDo.class, "today", new String[]{CURRENT_DATE});
        if (list.size() == 0 || list.isEmpty()) {
            StepDo data = new StepDo();
            data.setToday(CURRENT_DATE);
            data.setStep(tempStep + "");
            DbUtils.insert(data);
        } else if (list.size() == 1) {
            StepDo data = list.get(0);
            data.setStep(tempStep + "");
            DbUtils.update(data);
        } else {
        }
